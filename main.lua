...
function Speed_Library:SetNotification(Config)
  ...
  TextLabel2.Size = UDim2.new(1, -20, 0, 13 + (13 * (TextLabel2.TextBounds.X // math.max(TextLabel2.AbsoluteSize.X, 1))))
    ...
    function Sections:AddSection(Title, OpenSection)
      ...
      function Item:AddButton(Config)
        ...
        local function UpdateButtonSize()
          local _Height = 12 + (12 * (ButtonContent.TextBounds.X // math.max(ButtonContent.AbsoluteSize.X, 1)))
      ...
      function Item:AddToggle(Config)
        ...
        local function UpdateToggleSize()
          ...
          local Ratio = ToggleContent.TextBounds.X / math.max(ToggleContent.AbsoluteSize.X, 1)
      ...
      function Item:AddSlider(Config)
        ...
        function Funcs_Slider:Set(Value)
          ...
          TweenService:Create(SliderDraggable, TweenInfo.new(0.1, Enum.EasingStyle.Quad, Enum.EasingDirection.Out), { Size = UDim2.fromScale((Value - Min) / math.max(Max - Min, 1), 1) }):Play()
        end
        ...
        TextBox.FocusLost:Connect(function()
          if TextBox.Text ~= "" then
            Funcs_Slider:Set(tonumber(TextBox.Text) or Min)
            Callback(Funcs_Slider.Value)
          else
            Funcs_Slider:Set(0)
            Callback(Funcs_Slider.Value)
      ...
      function Item:AddDropdown(Config)
        ...
        local DropdownContent = Custom:Create("TextLabel", {
          ...
          Parent = Dropdown
				...
				DropdownContent.Size = UDim2.new(1, -180, 0, 12 + (12 * (DropdownContent.TextBounds.X // math.max(DropdownContent.AbsoluteSize.X, 1))))
